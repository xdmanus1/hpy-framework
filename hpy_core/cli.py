...
def main():
    ...
    pre_parser.add_argument("input_src", nargs="?", default=None)
...
""",
    ...
    parser.add_argument("input_src",metavar="SOURCE",nargs="?",default=argparse.SUPPRESS,
        help=f"Path to source .hpy file or directory.\n(default: '{effective_input_dir}' from {CONFIG_FILENAME} or built-in)")
    parser.add_argument("--init",metavar="PROJECT_DIR",
    ...
    parser.add_argument("-p","--port",metavar="PORT",type=int,default=8000,
    ...
    final_output_dir: str = args.output_dir if args.output_dir is not None else effective_output_dir
    ...
    if not is_directory_input and not is_file_input:
        print(f"Error: Input source '{final_input_src}' not found or not a file/directory.",file=sys.stderr)
        sys.exit(1)
    ...
    if is_directory_input:
        try:
            if output_dir_path.is_relative_to(input_path):
                print(f"Error: Output directory '{final_output_dir}' cannot be inside input directory '{final_input_src}'.",file=sys.stderr)
                sys.exit(1)
            ...
            try:
                ...
                print(f"Warning: Could not reliably check if output is inside input: {path_err}",file=sys.stderr)

    input_dir_context_abs = str(input_path.parent if is_file_input else input_path)
    error_count = 0
    print("--- Starting Build ---")
    print(f"Source: '{final_input_src}'{' (Directory)' if is_directory_input else ' (File)'}")
    print(f"Output: '{final_output_dir}'")
    if project_root:
        print(f"Config: Using '{CONFIG_FILENAME}' from '{project_root}'")

    is_dev_watch_mode = args.watch # Determine if watch mode is active for script injection
    ...
    if args.watch:
        ...
        watch_target_for_thread = str(input_path)
        input_dir_for_thread = input_dir_context_abs

        watcher_thread = threading.Thread(
            target=start_watching,
            args=(
                watch_target_for_thread,
                is_directory_input,
                input_dir_for_thread,
                str(output_dir_path),
                args.verbose,
            ),
            daemon=True,
        ...
        time.sleep(0.2) # Give watcher a moment
    ...
    if effective_serve:
        ...
        if not output_dir_path.is_dir():
            try:
                ...
                print(f"Error: Cannot create output directory '{final_output_dir}': {e}",file=sys.stderr)
                sys.exit(1)
        try:
            ...
            if watcher_thread and watcher_thread.is_alive():
                pass # Watcher is daemon, will exit with main thread
