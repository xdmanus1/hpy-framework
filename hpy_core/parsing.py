...
def parse_hpy_file(file_path: str, is_layout: bool = False) -> Dict[str, str]:
    ...
    if path.suffix.lower() != ".hpy":
        raise ValueError(f"Not a valid .hpy file: {file_path}")
    ...
    python_matches = re.findall(
        r"<python.*?>(.*?)</python>", content, re.DOTALL | re.IGNORECASE
    )
    ...
    if not html_match:
        raise ValueError(
            f"Error: Required <html>...</html> section not found in '{path.name}'."
        )
    ...
    if is_layout:
        if LAYOUT_PLACEHOLDER not in html_content_raw:
            raise ValueError(
                f"Error: Layout file '{path.name}' must contain placeholder '{LAYOUT_PLACEHOLDER}'."
            )
        if not python_matches:
            print(
                f"Warning: No <python> section found in layout '{path.name}'.",
                file=sys.stderr,
            )
        if not style_matches:
            print(
                f"Warning: No <style> section found in layout '{path.name}'.",
                file=sys.stderr,
            )
